 ...
 *
 * Declared here rather than imported from `@abfall-radar/api-client` for the same reason the capability shapes
 * are: this validator is used by the worker message contract and the schedule cache, both reachable from the
 * popup, and the popup must never reach the transport client. The transport package validates the links arriving
 * over HTTP; this validates the links crossing the extension's own boundaries, including values persisted by an
 * **older build** that had no such check.
 *
